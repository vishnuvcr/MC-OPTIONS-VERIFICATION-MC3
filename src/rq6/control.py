...
def simulate_terminal_paths(s0: float, historical_log_returns: np.ndarray, horizon: int = 3, paths: int = 5000, seed: int = 756, window: int = 756) -> np.ndarray:
    if s0 <= 0 or horizon <= 0 or paths <= 0:
    ...
    return s0 * np.exp(hist[sample_idx].sum(axis=1))

def terminal_quantiles(terminals: np.ndarray) -> dict[str, float]:
    return {"P20_PE": float(np.quantile(terminals, 0.20)), "P35_PE": float(np.quantile(terminals, 0.35)), "P65_CE": float(np.quantile(terminals, 0.65)), "P80_CE": float(np.quantile(terminals, 0.80))}
